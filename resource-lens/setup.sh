#!/usr/bin/env bash
#
# setup.sh — one-time prerequisite setup for CSTA Copilot (Resource Lens).
#
...
printf "\n${BOLD}CSTA Copilot — Resource Lens — setup${RESET}\n"
printf "${DIM}%s${RESET}\n\n" "$SCRIPT_DIR"
...
# --- 2. .env config --------------------------------------------------------
# Resource Lens needs no subdomain — the cookie itself targets confluent.cloud.
# The CONFLUENT_COOKIE value is captured automatically by ./run.sh (browser
# login), so we just make sure a .env exists.
if [ -f .env ]; then
  ...
  if [ -f .env.example ]; then
    ...
    printf 'CONFLUENT_COOKIE=\nCACHE_TTL_SECONDS=600\n' > .env
  fi
  note "$PASS" ".env created (CONFLUENT_COOKIE will be filled in by ./run.sh)"
fi
...
if [ -x "$VENV_PY" ] && [ -f requirements.txt ]; then
  if "$VENV_PY" -m pip --version >/dev/null 2>&1; then
    # Already satisfied?
    if "$VENV_PY" -c "import flask, requests, dotenv, playwright" >/dev/null 2>&1; then
...
# --- 5. Playwright browser (for cookie capture) ----------------------------
