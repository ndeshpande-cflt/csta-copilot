-- Ticket Lens.applescript — source for Ticket Lens.app (a stay-open applet).
--
-- Built into the bundle with:
--   osacompile -s -o "Ticket Lens.app" "Ticket Lens.applescript"
...
on idle
	-- Watch port 5001. Once we've seen the server up and then gone (Ctrl+C or
	...
	set isUp to (do shell script "lsof -tiTCP:5001 -sTCP:LISTEN >/dev/null 2>&1 && echo 1 || echo 0")
	if isUp is "1" then
		set wasUp to true
		set downTicks to 0
	else
...
on quit
	-- Stop the server when quit from the Dock. Killing the listener on port 5001
	-- is the most reliable (that's only ever our server). Escalate to SIGKILL.
	try
		do shell script "P=$(lsof -tiTCP:5001 -sTCP:LISTEN 2>/dev/null); if [ -n \"$P\" ]; then kill $P 2>/dev/null; sleep 1; P=$(lsof -tiTCP:5001 -sTCP:LISTEN 2>/dev/null); [ -n \"$P\" ] && kill -9 $P 2>/dev/null; fi; true"
	end try
	continue quit
end quit
